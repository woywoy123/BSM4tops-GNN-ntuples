...
void ParticleID::GetDecayPath(
	const xAOD::TruthParticle* top, 
	std::vector<const xAOD::TruthParticle*>* inpt)
{
	(*inpt).push_back(top); 
	...
	std::vector<int>* Path)
{
	...
	for (unsigned int i(0); i < particle -> nChildren(); ++i)
	{
		...
		if (!child){continue;}
		ParticleID::GetPath(child, index, Decay, Path); 
	}
	return particle; 
}
